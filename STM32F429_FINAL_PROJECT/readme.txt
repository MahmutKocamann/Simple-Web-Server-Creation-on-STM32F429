...
Bu örnek, DMA2D çevre biriminin Memory_to_Memory içindeki karıştırma aktarım moduyla nasıl yapılandırılacağına ilişkin bir açıklama sağlar.
 
Bu aktarım modunda iki giriş kaynağı kesilir: ön plan ve arka plan.

Bu örnekte, ön plan ve arka plan aşağıdaki gibi yapılandırılmıştır:

   - FLASH bellekte giriş belleği adresi
   - Renk modu: RGB565 formatı
   - Ön plan için sabit alfa, arka planı görmek üzere azaltılır.

   - Ön plan ve arka plan için alfa modu, ortaya çıkan görüntüde iki üst üste binmiş görüntüyü görecek şekilde yapılandırılmıştır.
        
  DMA2D, elde edilen pikseli hesaplamak için iki kaynak pikseli karıştırır.

  Çıkış belleği adresine aktarılan veriler, karıştırma işleminin sonucudur.
 
  Bu örnekte, katman 1, DMA2D çıkış arabelleğini görüntülemek üzere yapılandırılmıştır.

(ön plan ve arka planın karıştırılmasının sonucu)
 
Örnek çalıştırıldıktan sonra, LCD'de çekici farklı görüntü görüntülenir. Karışımın sonucunu görmek için kullanıcı düğmesini tıklayın.
